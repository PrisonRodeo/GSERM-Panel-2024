...
# called "VB" in the Environment window;
...
# you have the data stored "locally" in a subfolder
# called "Data".) 
#
...
SCOTUS <- read_csv("https://raw.githubusercontent.com/PrisonRodeo/GSERM-Panel-2024/main/Data/SCOTUS-votes.csv")
...
#
# R will of course do a *lot* of different
# statistical things. Most of what we'll be 
# doing in this class falls under the (broad)
# heading of "regression," and nearly all 
# regressions in R have a structure like:
#
# RESULT <- CMD(Y~X1+X2+X3,data=DATA,[options])
#
# where:
#
# - "RESULT" is the name of the object where
#    you store the statistical results, 
# - "CMD" is a command associated with a
#    particular regression model,
# - "Y" is the response / outcome / "dependent
#    variable,"
# - "X1," "X2," etc. are features / predictors /
#    "independent variables,"
# - "DATA" is the object (typically a data frame)
#    where Y, X1, X2, etc. are, and
# - [options] are options for model fitting, etc.
#
# So, for example, the command for a linear-additive
# regression model fit via OLS is "lm". We can fit 
# such a regression model on the SCOTUS data like
# this:

fit <- lm(LiberalPct~GOP,data=SCOTUS)

# Absent any additional commands, that doesn't
# seem to do much. You can show the resulting
# object:

fit

# ... but that just shows you the model and
# the estimated regression coeffients. A better 
# approach is to do a -summary()-:

summary(fit)

# Those are more conventional (and, hopefully,
# familiar) linear regression results.
#
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
# GETTING HELP                                    ####
#
